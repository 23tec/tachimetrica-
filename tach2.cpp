...
#define OPTO_PIN          2 // Foto accoppiatore su interrupt
...
const char M2[] PROGMEM = "Velocità raggiunta";
...
void setup() {
    ...
    // Non testato 
    // Full-wave
    // attachInterrupt( digitalPinToInterrupt( OPTO_PIN ), ac_on, CHANGE );
    // Half-wave
...
void loop() {
    ...
    if (motorEnabled && !alarm) {
        ...
        if (millis() - tachLastValid >= TACH_TIMEOUT 
            ...
            if (!motorAccel && pidInput - pidSetPoint <= -8.0) beep(100);
        }
    // Stop button    
    } else {
        // In decelerazione
