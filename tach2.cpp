...
bool AT_valid(char *cmd) {
    ...
    if (isValidNumber(v)) {
        if (strncmp(cmd, "at+kp=", 6) == 0) 
            kp = atof(v); return true;
        if (strncmp(cmd, "at+ki=", 6) == 0)
            ki = atof(v); return true;
        if (strncmp(cmd, "at+kd=", 6) == 0)
            kd = atof(v); return true;
    ...
    if (isValidNumber(v)) {
        if (strncmp(cmd, "at+accel=", 9) == 0) 
            rampAccel = atof(v); return true;
        if (strncmp(cmd, "at+decel=", 9) == 0)
            rampDecel = atof(v); return true;
    }
...
inline bool optoIsolatorFault() {
    return micros() - interruptEvent >= 10000; // 6000 full-wave
}
